...
if (!submitcheck('bgsubmit')) {
	$setting = array();
	$r= C::t('#xdep_boardbackground#gbg_setting')->fetch_by_all();
	foreach($r as $s){
...
EOT;
	...
	showtips(lang('plugin/xdep_boardbackground', 'Setting_cong').lang('plugin/xdep_boardbackground', 'Setting_con'));
	showtableheader(lang('plugin/xdep_boardbackground', 'Setting_titleg'));
	showsubtitle(array('', lang('plugin/xdep_boardbackground', 'Setting_gid'), lang('plugin/xdep_boardbackground', 'Setting_bgcolor'),lang('plugin/xdep_boardbackground', 'Setting_imgurl'),lang('plugin/xdep_boardbackground', 'Setting_xr'),lang('plugin/xdep_boardbackground', 'Setting_yr'),lang('plugin/xdep_boardbackground', 'Setting_xc'),lang('plugin/xdep_boardbackground', 'Setting_posts'),lang('plugin/xdep_boardbackground', 'Setting_fixed')));
	...
	showformfooter();
		...
		for($i=0;$i<count($tionnewadd['fid']);$i++){
			if(!empty($tionnewadd['fid'][$i])){
				C::t('#xdep_boardbackground#gbg_setting')->insert($tionnewadd['fid'][$i],$tionnewadd['color'][$i],$tionnewadd['imgurl'][$i],$tionnewadd['xrepeat'][$i],$tionnewadd['yrepeat'][$i],$tionnewadd['xcenter'][$i],$tionnewadd['posts'][$i],$tionnewadd['fixed'][$i]);
				}
			}
		$t=array_keys($tion);
		foreach($t as $i){
			C::t('#xdep_boardbackground#gbg_setting')->update_by_value($i,$tion[$i]['fid'],$tion[$i]['color'],$tion[$i]['imgurl'],$tion[$i]['xrepeat'],$tion[$i]['yrepeat'],$tion[$i]['xcenter'],$tion[$i]['posts'],$tion[$i]['fixed']);
			}
		foreach($delete as $d){
			C::t('#xdep_boardbackground#gbg_setting')->delete_by_id($d);
			}
